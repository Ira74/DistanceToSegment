...
namespace DistanceTask
{
	internal static class Program
	{
		...
		{
			yield return new DistanceTestRoomTestCase(new Point(d, d), new Point(d, d), new Point(0, 0), d * Math.Sqrt(2));
		...
		private static IEnumerable<DistanceTestRoomTestCase> OutsideSegment(int d)
		{
			...
			yield return new DistanceTestRoomTestCase(new Point(-d, 0), new Point(d, 0), new Point(d, d), d);
			yield return new DistanceTestRoomTestCase(new Point(-d, 0), new Point(d, 0), new Point(2 * d, d), d * Math.Sqrt(2));
	...
	internal class DistanceTestRoomTestCase : TestCase
	{
		...
		protected override void InternalVisualize(TestCaseUI ui) //граф часть
		{
